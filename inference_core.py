...
class InferenceCore:
    ...
    def do_pass(self, key_k, key_v, idx, end_idx,shrinkage):
        self.mem_bank.add_memory(key_k, key_v,shrinkage)
        closest_ti = end_idx
        ...
        for ti in this_range:
            k16, qv16, qf16, qf8, qf4,ms,qe = self.encode_key(ti)
            out_mask = self.prop_net.segment_with_query(self.mem_bank, qf8, qf4, k16, qv16,qe)
            ...
            if ti != end:
                ...
                if self.include_last or is_mem_frame:
                    ...
                    ms = ms.unsqueeze(2)
                    self.mem_bank.add_memory(prev_key, prev_value, ms,is_temp=not is_mem_frame)
    ...
    def interact(self, mask, frame_idx, end_idx):
        ...
        key_k, _, qf16, _, _,shrinkage,_ = self.encode_key(frame_idx)
        ...
        # Propagate
        self.do_pass(key_k, key_v, frame_idx, end_idx,shrinkage)
